...
inline vec3 random_unit_vector() {
    while (true) {
        ...
        if ( 1e-160 < lensq && lensq <= 1)
            //normalizing p to unit vector if not already
            return p = p / sqrt(lensq);
    }
}

inline vec3 random_on_hemisphere(const vec3& normal) {
    vec3 on_unit_sphere = random_unit_vector();
    if (dot(on_unit_sphere, normal) > 0.0)  //Good vector, same hemisphere as normal
